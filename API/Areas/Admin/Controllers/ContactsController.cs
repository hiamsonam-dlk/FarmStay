...
namespace API.Areas.Admin.Controllers
{
    ...
    {        
        ...
        public IActionResult SaveItem(string Id=null,int Type=0)
        {
            ...
            data.SearchData = new SearchContacts() { CurrentPage = 0, ItemsPerPage = 10, Keyword = ""};            
            if (IdDC == 0)
            {
                data.Item = new Contacts() { CreatedDateShow = DateTime.Now.ToString("dd/MM/yyyy HH:mm"), Type=Type};
            }
            else {
        ...
        {
            string ControllerName = this.ControllerContext.RouteData.Values["controller"].ToString();
            Contacts model = new Contacts() { Id = Int32.Parse(MyModels.Decode(Id, API.Models.Settings.SecretId + ControllerName).ToString()) };            
            try
            {
                if (model.Id > 0)
                {
        ...
        {
            string ControllerName = this.ControllerContext.RouteData.Values["controller"].ToString();
            Contacts item = new Contacts() { Id = Int32.Parse(MyModels.Decode(Ids, API.Models.Settings.SecretId + ControllerName).ToString()), Status = Status };
            try
            {
                if (item.Id > 0)
                {

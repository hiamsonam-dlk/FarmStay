...
namespace API.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult SiteVisit()
        {
            try
            {
                if (string.IsNullOrEmpty(HttpContext.Request.Cookies["VISITED"]))
                {
                    API.Areas.Admin.Models.SiteVisit.SiteVisitService.SaveItem(DateTime.Now.ToString("yyyyMMdd"));
                    HttpContext.Response.Cookies.Append("VISITED", Guid.NewGuid().ToString(), new CookieOptions { Expires = DateTime.Now.AddDays(0.8) });
                }
        ...
        public async Task<IActionResult> Import()
        {
            ...
            data.ListItems = new List<Articles>() { };
            data.ListItems = ArticlesService.GetList();

            var ListRelated = await ImportService.GetListAll();

            foreach (var item in data.ListItems)
            {
                var fulltext = (from t in ListRelated where t.Params == item.Params  select t).FirstOrDefault();
                item.FullText = fulltext.FullText;
                ArticlesService.UpdateFulltext(item.Id,item.FullText);

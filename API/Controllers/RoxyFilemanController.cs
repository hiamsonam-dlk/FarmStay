...
namespace API.Controllers
{
    ...
    public class RoxyFilemanController : Controller
    {
        ...
        public ActionResult DOWNLOAD(string f)
        {
            try
            {
                //f = ManagerFilesService.ValidateFileName(f);
        ...
        public ActionResult DOWNLOADDIR(string d)
        {
            try
            {
                d = ManagerFilesService.ValidateFileName(d);
        ...
        public IActionResult RENAMEFILE(string f, string n)
        {
            try
            {
                ...
                if (flagSave)
                {
                    ...
                    throw new Exception("Tên file không hợp lệ; " + LangRes("E_RenameFile") + " \"" + f + "\"");
                }
        ...
        */
        ...
        private string MakeVirtualPath(string path)
        {
            ...
            if (Login != null && Login != "")
            {
                ...
                if (MyInfo.IdGroup > 1)
                {
                    _filesRootVirtual = "/CMS/Content/User/" + MyInfo.Id.ToString();
                    _filesRootPath = "/wwwroot/CMS/Content/User/" + MyInfo.Id.ToString();
                }
        ...
        {
            string newPath = ManagerFilesService.ValidatePath(path);
        ...
        {
            path = ManagerFilesService.ValidatePath(path);
            path = path.TrimStart('~');
